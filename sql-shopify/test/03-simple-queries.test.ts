...
describe("Simple Queries", () => {
    ...
    it("should select top 3 develepors with most apps published", async done => {
        const query = 
        `SELECT COUNT(developer) AS count,developer
        FROM apps
        GROUP BY developer
    ...
    it("should select count of reviews created in year 2014, 2015 and 2016", async done => {
        const query = 
        `SELECT substr(date_created,-4,4)as year,COUNT(*)AS review_count
        FROM reviews
        GROUP BY year HAVING year='2014' OR year='2015' OR year='2016'`;
        const result = await db.selectMultipleRows(query);
        expect(result).toEqual([
