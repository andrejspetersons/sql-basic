...
import { App, AppCategory, Category, KeyBenefit, PricingPlan, PricingPlanPrice, Review } from "../src/data/shopify-types";
...
import { APPS, APPS_CATEGORIES, APPS_PRICING_PLANS, CATEGORIES, KEY_BENEFITS, PRICING_PLANS, REVIEWS } from "../src/shopify-table-names";
...
const insertApps = (apps: App[]) => {
    return (
        `insert into apps (
            id,url,title,tagline,developer,developer_link,icon,rating,reviews_count,description,pricing_hint) values` +
        apps.map(app => `(${app.shopifyAppId},'${escape(app.url)}','${escape(app.title)}','${escape(app.tagline)}','${escape(app.developer)}','${escape(app.developerLink)}','${escape(app.icon)}',${app.rating},${app.reviewsCount},'${escape(app.description)}','${escape(app.pricingHint)}')`).join(",")
    );
};
